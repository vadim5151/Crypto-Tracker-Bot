...
def format_daily(today_events, tomorrow_events):
    ...
    if today_events and today_events[0] != 'Событий не запланировано':
        ...
        important_today = [event for event in today_events if event['importance'] >= 2][:3]
    ...
    if tomorrow_events and tomorrow_events[0] != 'Событий не запланировано':
        ...
        important_tomorrow = [event for event in tomorrow_events if event['importance'] >= 2][:3]
...
def format_events_today(events, offset=0, limit=20, max_length=4096):
    if not events:
        return ["📅 <b>На сегодня:</b> нет событий (Попробуйте позже, возможно, данные сейчас собираются)"]
...
def format_events_tomorrow(events, offset=0, limit=20, max_length=4096):
    if not events:
        return ["📅 <b>На завтра:</b> нет событий (Попробуйте позже, возможно, данные сейчас собираются)"]
...
def format_events_week(events, offset=0, limit=20, max_length=4096):
    if not events:
        return ["📅 <b>На неделе:</b> нет событий (Попробуйте позже, возможно, данные сейчас собираются)"]
...
def sort_today_events(events, importance, offset=0, limit=20):
    if not events:
        return f"📅 <b>На сегодня:</b> нет событий с важностью {'⭐' * importance}"
...
def sort_tomorrow_events(events, importance, offset=0, limit=20):
    if not events:
        return f"📅 <b>На завтра:</b> нет событий с важностью {'⭐' * importance}"
...
def sort_week_events(events, importance, offset=0, limit=20):
    if not events:
        return f"📅 <b>На неделе:</b> нет событий с важностью {'⭐' * importance}"
